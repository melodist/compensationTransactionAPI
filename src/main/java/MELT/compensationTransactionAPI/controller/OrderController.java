...
import MELT.compensationTransactionAPI.domains.OrderStatus;
...
public class OrderController {
    ...
    public ResponseEntity<Message<OrderDto>> createOrder(@PathVariable Long id, @PathVariable int count) {
        ...
        Long orderId = orderService.createOrder(id, count);
        log.debug("orderId: {}", orderId);

        if (orderId > 0) {
            OrderDto orderDto = new OrderDto(orderId, OrderStatus.APPROVED, id, count);
            message = new Message<>("200 OK", "주문에 성공하였습니다.", orderDto);
            return new ResponseEntity<>(message, HttpStatus.OK);
        } else {
            OrderDto orderDto = new OrderDto(-orderId, OrderStatus.CANCEL, id, count);
